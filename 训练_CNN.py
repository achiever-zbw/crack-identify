...
scheduler = lr_scheduler.StepLR(
    optimizer,
    step_size=40,  # 每40个epoch调整一次
    gamma=0.3  # 学习率降为原来的0.3倍
)
...
# 保存模型
torch.save(model.state_dict(), 'trained_model_CNN_4.pth')
print("模型已保存")


# 绘制损失变化图像并保存
plt.plot(range(1, 171), losses,
...
plt.legend()
# 保存图像到文件
plt.savefig('1.12_CNN_4.png')  # 将图像保存为 .png 文件
print("损失曲线图已保存")
