...
model = MLP(input_data, hide_data, output_data)  # 创建对象 使用：model.方法名
# 定义损失函数和优化器
...
for each in range(num_epochs):
    ...
    for i, (inputs, labels) in enumerate(dataloader, 0):
        ...
        inputs = inputs.view(inputs.size(0), -1)  # 转化为模型期望的形状，一维化
...
# 保存图像到文件
plt.savefig('12.24_13pm_.png')  # 将图像保存为 .png 文件
print("损失曲线图已保存")

# 保存模型
torch.save(model.state_dict(), 'trained_model_mlp.pth')
print("模型已保存")
